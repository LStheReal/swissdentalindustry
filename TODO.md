...
- 2026-09-24 (3) — Eigentlicher Fehler war das Logo-Laufband auf der Startseite (V2Marquee), nicht die Mitgliederseite. Ursachen: Items ohne eigene Breite (0px bis Bild geladen) + loading=lazy (per transform bewegte Logos lädt Safari spät/nie → Lücken, Band springt) + Safari-Bildgrösse nach Originaldatei + Band 14'292px breit (iOS verwirft Teile breiter animierter Ebenen) + gap machte -50% ungenau. Fix 11515e9: feste Box 160px (Handy 120px), object-fit contain, eager, Wiederholung auf ~16 Logos/Hälfte, margin statt gap. Live, Geometrie geprüft (32/32 geladen, Schleife exakt); Sichttest mit laufender Animation nicht möglich (Browser-Pane ausgeblendet, Chrome-Extension offline) → auf iPhone + Desktop prüfen.
- 2026-09-24 (4) — Louise: Logo-Band hässlich, nicht alle Logos, alles mehrfach doppelt. Neu (2054e45): alle Mitglieder als weisse Kacheln, Logos farbig, auf zwei gegenläufigen Reihen (jede Runde breiter als jeder Bildschirm → kein Logo doppelt im Bild), Zähler darunter; Handy 118×70, ab sm 176×100 mit Hover-Lift + Pause; reduzierte Bewegung = ruhiges Raster. Altes V2Marquee entfernt. Live auf Handy + Desktop geprüft (70/70 Bilder geladen, Reihen laufen gegenläufig, kein Überlauf).
- 2026-09-24 (5) — Louise sah das Logo-Band als feste 2-Spalten-Liste ohne Bewegung. In WebKit (Playwright, iPhone 14) nachgestellt: exakt das Ersatz-Raster bei prefers-reduced-motion (18 Zeilen, 1482px) → ihr Gerät meldet "Bewegung reduzieren" (sie meint, es sei aus). Fix a7d671b: bei reduzierter Bewegung dieselben zwei Reihen zum Wischen (scroll-snap, 170px hoch). WebKit-Check live: normal iPhone/Desktop laufen gegenläufig, reduziert 2 wischbare Reihen, alle Logos geladen, kein Überlauf. Offen: bei Louise Einstellung prüfen; falls Band trotzdem immer laufen soll → reduced-motion-Block anpassen.
- [ ] Louise: "Bewegung reduzieren" am iPhone prüfen; entscheiden, ob Band trotzdem immer laufen soll
...
- [x] 2026-08-23 Test-Mitglied „louise" gelöscht (stand veröffentlicht im Verzeichnis). Cascade hat Kontakt, Edit-Token und Änderungsanfrage mitgenommen — geprüft, 0 Restzeilen. Stand jetzt: **35 Mitglieder, alle published, 29 mit Website**. Sicherung: scratchpad/louise-backup.json. Zugehöriger Antrag und Logo-Datei ebenfalls gelöscht (2026-08-23). Beim Sichern der Logo-Datei aufgefallen: **genau diese eine Datei lag korrupt im Bucket** — PNG-Signatur 89 50 4E 47 war zu EF BF BD 50 4E 47 verstümmelt (UTF-8-Ersatzzeichen statt 0x89), also irgendwann durch eine Text-Dekodierung gelaufen. Daraufhin alle 35 Logos geprüft: 30 PNG/JPG/SVG + 5 WebP, **alle gültig**. Der Defekt war auf diese Testdatei beschränkt, kein Fehler im Upload-Pfad.
